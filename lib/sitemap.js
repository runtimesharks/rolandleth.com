...
class SiteMap {
	...
	static create() {
		...
		urls.forEach(function(url) {
			const priority = highPriority.includes(url) ? 0.9 : lowPriority.includes(url) ? 0.3 : 0.1
			xml += "<url>"
		...
		return Db.fetchSiteMapPosts().then(function(data) {
			data.posts.forEach(function(post) {
				const datetime = Post.datetimeFromDate(post.modified).slice(0, -5)
