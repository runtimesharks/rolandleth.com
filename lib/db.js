			...
			client.query(query, function(err, result) {
				...
				res.posts = posts.map(function(rawPost) {
					...
					if (config.searching) {
						...
						// Not perfect, but I can't find a better way, for now.
						// If there are two occurrences on the same paragraph (a 'markdown line'), all terms will get unmarked.
						let lines = []
						body.split('\n').forEach(function(line) {
							let _line = line
							if (line.indexOf('<a href="') > -1 ||
							    line.indexOf('/assets/') > -1 ||
							    line.indexOf('<img src="') > -1) {
								_line = _line.replace(/<\/mark>|<mark class='search'>/g, '')
							}
							lines.push(_line)
						})
						body = lines.join('\n')
