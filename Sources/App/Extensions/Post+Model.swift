...
extension Post: Model {
	...
	func makeRow() throws -> Row {
		...
		try row.set("firstparagraph", firstParagraph)
		try row.set("datetime", datetime)
		try row.set("date", date)
		try row.set("isodate", isoDate)
...
extension Post: Preparation {
	...
	static func prepare(_ database: Database) throws {
		try database.create(self) { posts in
			...
			posts.string("firstparagraph", length: 300, optional: false, unique: false, default: nil)
			posts.string("datetime", length: 15, optional: false, unique: false, default: nil)
			posts.string("isodate", length: 16, optional: false, unique: false, default: nil)
